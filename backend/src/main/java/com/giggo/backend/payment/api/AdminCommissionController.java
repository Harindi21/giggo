...
public class AdminCommissionController {
    ...
    public ApiResponse<CommissionResponse> set(
        ...
        return ApiResponse.ok(commissionService.setRate(categoryId, req.rate()));
    }

    @DeleteMapping("/{categoryId}")
    @PreAuthorize("hasRole('ADMIN')")
    public ApiResponse<CommissionResponse> clear(@PathVariable UUID categoryId) {
        return ApiResponse.ok(commissionService.clearRate(categoryId));
    }
}
